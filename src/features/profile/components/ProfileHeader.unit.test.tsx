...
describe('ProfileHeader', () => {
	...
	it('orders each definition term before its value in the DOM', () => {
		renderHeader();

		const followersTerm = screen.getByText('ProfileHeader.followers');
		const followerCount = screen.getByText('12');
		const followingTerm = screen.getByText('ProfileHeader.following');
		const followingCount = screen.getByText('8');

		expect(followersTerm.nextElementSibling).toBe(followerCount);
		expect(followingTerm.nextElementSibling).toBe(followingCount);
	});
